...
const day19_coding: CodingQuestion[] = [{
  id: "code-d19-01", title: "Serialize and Deserialize Binary Tree", difficulty: "Hard", description: "Design an algorithm to convert a binary tree into a string, and reconstruct the exact same tree back from that string.", examples: [{ input: "root = [1, 2, 3, null, null, 4, 5]", output: "The same tree, after being round-tripped through serialize() then deserialize()", explanation: "The pre-order traversal '1,2,#,#,3,4,#,#,5,#,#' encodes both structure and nulls, so it can be rebuilt exactly." }], constraints: ["0 <= number of nodes <= 10^4", "Node values can be any integer, including negative", "Must handle an empty tree (root = null)"], hint: "Use pre-order DFS, writing '#' for every null child so the string alone captures the full shape. Deserialize by consuming tokens in the same pre-order sequence.", solution: `function serialize(root) {\n  const result = [];\n  function dfs(node) {\n    if (!node) { result.push('#'); return; }\n    result.push(String(node.val));\n    dfs(node.left);\n    dfs(node.right);\n  }\n  dfs(root);\n  return result.join(',');\n}\n\nfunction deserialize(data) {\n  const tokens = data.split(',');\n  let i = 0;\n  function dfs() {\n    const token = tokens[i]; i++;\n    if (token === '#') return null;\n    const node = { val: Number(token), left: null, right: null };\n    node.left = dfs();\n    node.right = dfs();\n    return node;\n  }\n  return dfs();\n}`
}];
const day20_coding: CodingQuestion[] = [{
  id: "code-d20-01", title: "Find Missing Positive", difficulty: "Hard", description: "Given an unsorted array of integers, return the smallest missing positive integer, in O(n) time and O(1) extra space.", examples: [{ input: "[1, 2, 0]", output: "3" }, { input: "[3, 4, -1, 1]", output: "2" }, { input: "[7, 8, 9, 11, 12]", output: "1", explanation: "1 never appears, so it's the smallest missing positive." }], constraints: ["1 <= nums.length <= 300", "O(n) time, O(1) auxiliary space — cannot use a Set or sort"], hint: "Place each value v (where 1 <= v <= n) at index v-1 by swapping, in place. Then scan for the first index whose value doesn't match index+1 — that index+1 is the answer.", solution: "function firstMissingPositive(nums) {\n  const n = nums.length;\n  for (let i = 0; i < n; i++) {\n    while (nums[i] > 0 && nums[i] <= n && nums[nums[i] - 1] !== nums[i]) {\n      const target = nums[i] - 1;\n      [nums[i], nums[target]] = [nums[target], nums[i]];\n    }\n  }\n  for (let i = 0; i < n; i++) {\n    if (nums[i] !== i + 1) return i + 1;\n  }\n  return n + 1;\n}"
}];
const day21_coding: CodingQuestion[] = [{
  id: "code-d21-01", title: "Light Debugging", difficulty: "Easy", description: "The following loop was supposed to log 0, 1, 2, 3, 4 (one per line) but logs 5 five times instead. Fix it with a minimal change.\\n\\nfor (var i = 0; i < 5; i++) { setTimeout(() => console.log(i), 100); }", examples: [{ input: "for (var i = 0; i < 5; i++) { setTimeout(() => console.log(i), 100); }", output: "5\\n5\\n5\\n5\\n5", explanation: "var is function-scoped, so all five callbacks close over the SAME i, which has already reached 5 by the time any callback runs." }, { input: "for (let i = 0; i < 5; i++) { setTimeout(() => console.log(i), 100); }", output: "0\\n1\\n2\\n3\\n4", explanation: "let is block-scoped — each loop iteration gets its own fresh binding of i, so each closure captures a different value." }], constraints: ["Fix must not restructure the loop — only the var/let keyword should change"], hint: "Swap var for let so each iteration gets its own binding of i, captured correctly by each closure.", solution: "for (let i = 0; i < 5; i++) { setTimeout(() => console.log(i), 100); }"
}];
