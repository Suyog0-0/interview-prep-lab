...
const coding: CodingQuestion[] = [
  {
    ...
    difficulty: "Easy",
    description: "Given an array and an integer k, find the maximum sum of any contiguous subarray of exactly length k.",
  ...
  },
  {
    ...
    difficulty: "Easy",
    description: "Given an array and an integer k, find the contiguous subarray of length k with the maximum average value.",
  ...
  },
  {
    ...
    constraints: ["1 <= nums.length <= 10^5", "1 <= nums[i] <= 10^4", "All values are positive — required for the shrink logic to be valid"],
  ...
  },
  {
    ...
    difficulty: "Easy",
    description: "Given a binary array, return the maximum number of consecutive 1s.",
  ...
  },
  {
    ...
    difficulty: "Medium",
    description:
      "An array represents fruit types on a row of trees. You have exactly two baskets, each can hold only one type of fruit but unlimited quantity. Find the length of the longest contiguous run of trees you can pick from using at most two distinct fruit types.",
    ...
    hint: "Variable window with a frequency Map. Shrink from the left whenever the Map holds more than 2 distinct keys.",
    solution:
      "function totalFruit(fruits) {\n  const basket = new Map(); // fruit type -> count in window\n  let left = 0;\n  let max = 0;\n  for (let right = 0; right < fruits.length; right++) {\n    basket.set(fruits[right], (basket.get(fruits[right]) || 0) + 1);\n    while (basket.size > 2) {\n      const leftType = fruits[left];\n      basket.set(leftType, basket.get(leftType) - 1);\n      if (basket.get(leftType) === 0) basket.delete(leftType);\n      left++;\n    }\n    max = Math.max(max, right - left + 1);\n  }\n  return max;\n}",
  },
];
...
const mcqs: MCQQuestion[] = [
  {
    ...
    options: [
      "JavaScript arrays cache their sums automatically",
      "You subtract the element leaving and add the element entering, instead of re-summing all k elements",
      "The array is always small",
      "You use a Set instead of a running sum",
    ],
    correctAnswerIndex: 1,
    explanation:
      "Recomputing the sum of every new window from scratch is O(k) per slide, O(nk) overall. Updating the running total by removing the outgoing element and adding the incoming one keeps each slide O(1), giving O(n) total.",
  },
  {
    id: "s20-m02",
    ...
    options: [
      ...
      "Negative numbers cause a syntax error",
  ...
  },
  {
    ...
    correctAnswerIndex: 1,
    explanation:
      "Two basket types map directly onto 'at most 2 distinct values in the window'. The same Map-based variable-window technique generalises to any K by changing the shrink condition to `basket.size > K`.",
  },
];
...
export const s20_sliding_window: InterviewSection = {
  ...
  subtitle: "Fixed and variable windows over arrays and strings",
  color: "#0ea5e9",
