...
const notes: NoteSection[] = [
  {
    ...
    content:
      "Binary search halves the search space every iteration by comparing the middle element to the target, giving O(log n). The three things that go wrong most often: an infinite loop from an incorrect boundary update, an off-by-one from using <= vs < inconsistently, and integer overflow from (low + high) / 2 in fixed-width languages (rare in JS, but interviewers expect the safer form anyway).",
    code: "function binarySearch(arr, target) {\n  let low = 0, high = arr.length - 1;\n  while (low <= high) {\n    const mid = low + Math.floor((high - low) / 2);\n    if (arr[mid] === target) return mid;\n    if (arr[mid] < target) low = mid + 1;\n    else high = mid - 1;\n  }\n  return -1;\n}",
    language: "javascript",
    tip: "The array does not have to be literally sorted — it just needs a 'search space' where you can decide, from one comparison, which half to discard. That generalises binary search to 'search on the answer' problems.",
  },
];
...
const coding: CodingQuestion[] = [
  {
    ...
    title: "Binary Search",
    difficulty: "Easy",
    description: "Given a sorted array and a target, return its index or -1 if absent.",
  ...
  },
  {
    ...
    difficulty: "Easy",
    description: "Given a sorted array and a target, return the index where the target is found, or where it would be inserted to keep the array sorted.",
    examples: [
  ...
  },
  {
    ...
    hint: "This is literally binary search where the 'comparison' is a function call instead of an array read.",
    solution:
      "function guessNumber(n, guess) {\n  let low = 1;\n  let high = n;\n  while (low <= high) {\n    const mid = low + Math.floor((high - low) / 2);\n    const result = guess(mid);\n    if (result === 0) return mid;\n    if (result < 0) high = mid - 1; // mid was too high\n    else low = mid + 1;             // mid was too low\n  }\n  return -1;\n}",
  },
  {
    id: "s22-c04",
    ...
    difficulty: "Easy",
    description:
      "You have n versions [1..n] and a function isBadVersion(version) that returns true once a version is bad; every version after the first bad one is also bad. Find the first bad version with the minimum number of calls.",
    examples: [{ input: "n = 5, bad = 4", output: "4" }],
    constraints: ["Minimize calls to isBadVersion — O(log n) required"],
    hint: "This is 'find the boundary in a sorted true/false array' — binary search where the condition is monotonic (false...false, then true...true).",
    solution:
      "function firstBadVersion(n, isBadVersion) {\n  let low = 1;\n  let high = n;\n  while (low < high) {\n    const mid = low + Math.floor((high - low) / 2);\n    if (isBadVersion(mid)) high = mid;      // bad version could still be the answer, keep it in range\n    else low = mid + 1;                     // mid is good, answer is strictly after it\n  }\n  return low;\n}",
  },
  {
    id: "s22-c05",
...
const mcqs: MCQQuestion[] = [
  ...
  {
    id: "s22-m03",
    question: "In Search in Rotated Sorted Array, how do you know which half of the array is 'normally sorted' at each step?",
    options: [
